                ...
                if (nonceStr == "test" && desc != "0") {
                    ...
                    qb = await this.entityManager.transaction(async (entityManager) => {
                        let proxyCharging = await entityManager.createQueryBuilder(proxyChargin_entity_1.ProxyCharging, "proxyCharging")
                            ...
                            .getOne();
                        if (proxyCharging) {
                            await entityManager.query(`update proxy_charging set status = 2,locking = 1  where id = ${proxyCharging.id}`);
    ...
    checkOrder() {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (!info.isUse)
                    return Promise.resolve();
                console.log(`${process.pid}执行查余额代充定时查单处理`);
                ...
                for (let i = 0; i < orders.length; i++) {
                    ...
                    if (!orderInfo) {
                        ...
                        if (orderRedis.phoneBalance) {
                            ...
                            console.log("尚未查询到第一次余额 ，不执行改变后的查询");
                        }
                    }
                }
                resolve();
